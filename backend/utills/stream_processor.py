...
class BuilderToolBase:
    def __init__(
        ...
        toolName: str,
        displayTitle: str,
        getCustomDisplayTitle=None,
    ):
        self.toolName = toolName
        self.displayTitle = displayTitle
        self.getCustomDisplayTitle = getCustomDisplayTitle


class BuilderTool(BuilderToolBase):
    def __init__(self, tool, **kwargs):
        super().__init__(**kwargs)
        self.tool = tool


# Stream chunk typing (logical aliases)
AgentMessageChunk = Dict[str, Any]
ToolProgressChunk = Dict[str, Any]
WorkflowUpdateChunk = Dict[str, Any]
StreamOutput = Dict[str, List[Dict[str, Any]]]
...
def has_message_in_update(update: Any) -> bool:
    return (
        ...
        and len(update["messages"]) > 0
    )


def should_filter_subgraph_update(
    namespace: List[str],
    data: Dict[str, Any],
) -> bool:
    if not is_from_skipped_subgraph(namespace):
...
def process_operations_update(update: Any) -> Optional[StreamOutput]:
    ...
    chunk: WorkflowUpdateChunk = {
        "role": "assistant",
        "type": "workflow-updated",
        "codeSnippet": json_dumps(update["workflowJSON"]),
    }

    return {"messages": [chunk]}


def process_agent_node_update(
    node_name: str,
    update: Any,
) -> Optional[StreamOutput]:
    if not should_emit_from_node(node_name):
        return None
...
def format_human_message(msg: HumanMessage) -> Dict[str, Any]:
    raw_text = extract_human_message_text(msg.content)
    cleaned = clean_context_tags(raw_text)
...
def process_ai_message_content(
    msg: AIMessage,
) -> List[Dict[str, Any]]:
    if not msg.content:
        return []
    ...
    if isinstance(msg.content, list):
        return process_array_content(msg.content)
...
def create_tool_call_message(
    tool_call: ToolCall,
    builder_tool: Optional[BuilderToolBase],
) -> Dict[str, Any]:
    return {
        "id": tool_call.id,
        "toolCallId": tool_call.id,
        ...
        "displayTitle": builder_tool.displayTitle if builder_tool else None,
        "customDisplayTitle": (
            builder_tool.getCustomDisplayTitle(tool_call.args)
            if builder_tool and tool_call.args
            else None
        ),
        "status": "completed",
        "updates": [{
            "type": "input",
            "data": tool_call.args or {},
        }],
    }
...
def process_tool_calls(
    ...
    builder_tools: Optional[List[BuilderToolBase]],
) -> List[Dict[str, Any]]:
    result = []
    ...
    for call in tool_calls:
        ...
        if builder_tools:
            builder_tool = next(
                (bt for bt in builder_tools if bt.toolName == call.name),
                None,
            )

        result.append(create_tool_call_message(call, builder_tool))
...
def process_tool_message(
    ...
    for i in range(len(formatted_messages) - 1, -1, -1):
        ...
        if m.get("type") == "tool" and m.get("id") == tool_call_id:
            m.setdefault("updates", []).append({
                "type": "output",
                "data": (
                    {"result": msg.content}
...
def format_messages(
    ...
    for msg in messages:
        if isinstance(msg, HumanMessage):
            ...
            if msg.tool_calls:
                formatted.extend(
                    process_tool_calls(msg.tool_calls, builder_tools)
                )

        elif isinstance(msg, ToolMessage):
            process_tool_message(msg, formatted)
